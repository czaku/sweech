...
describe('loadEstate', () => {
  ...
  it('merges official sweech subscription profiles into the estate', async () => {
    ...
    const sweechPath = join(dir, 'sweech.json');

    await writeFile(estatePath, VALID_YAML);
    await writeFile(sweechPath, JSON.stringify([
      { commandName: 'claude-pole', cliType: 'claude', provider: 'anthropic' },
      { commandName: 'codex-pole', cliType: 'codex', provider: 'openai' },
      { commandName: 'kimi-work', cliType: 'claude', provider: 'kimi' },
    ]));

    const estate = await loadEstate(estatePath, sweechPath);
