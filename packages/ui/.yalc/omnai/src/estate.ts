...
export function getSweechConfigPath(): string {
  return join(homedir(), '.sweech', 'config.json');
}

export async function loadEstate(path?: string, sweechPath?: string): Promise<Estate> {
  const filePath = path ?? getEstatePath();
  const content = await readFile(filePath, 'utf-8');
  const estate = parseAndValidate(content);
  return mergeSweechProfiles(estate, await loadSweechProfiles(sweechPath));
}
...
export async function loadSweechProfiles(path?: string): Promise<SweechProfile[]> {
  const filePath = path ?? getSweechConfigPath();
  try {
    const content = await readFile(filePath, 'utf-8');
    const raw = JSON.parse(content);
    return Array.isArray(raw) ? raw as SweechProfile[] : [];
  } catch {
    return [];
  }
}
